...
export const LivingCostEstimator: React.FC<Props> = ({ costs, onCostsChange, totalDuration, totalLivingCost, pathwayDetails }) => {
    const durationBreakdownText = pathwayDetails.length > 1
        ? `(${pathwayDetails.map(p => `${p.courseAbbreviation} ${p.durationMonths}个月`).join(' + ')})`
        : '';

    return (
        <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">生活费估算 (可选)</h3>
            <div className="p-3 mb-4 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-800 flex items-start gap-2">
                ...
                <span>这些仅为估算值，不属于课程费用的一部分。费用根据所选课程的总时长计算。</span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                <CostInput label="房租 / 月" value={costs.rent} onChange={(v) => onCostsChange({...costs, rent: v})} />
                <CostInput label="伙食 / 月" value={costs.food} onChange={(v) => onCostsChange({...costs, food: v})} />
                <CostInput label="交通 / 月" value={costs.transport} onChange={(v) => onCostsChange({...costs, transport: v})} />
                <CostInput label="医疗 / 月" value={costs.medical} onChange={(v) => onCostsChange({...costs, medical: v})} />
            </div>
             <div className="mt-4 pt-4 border-t border-gray-200">
                ...
                <div className="flex justify-between items-start text-gray-600 text-sm">
                    ...
                    <div className="text-right">
                        <span className="font-medium text-gray-800">{totalDuration} 个月</span>
                ...
                <div className="flex justify-between items-center mt-2 font-bold text-base">
                    <span className="text-gray-900">预估生活费总额:</span>
