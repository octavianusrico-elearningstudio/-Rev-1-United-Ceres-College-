...
export const FeeBreakdown: React.FC<Props> = ({ course, studentType }) => {
    ...
    const studentTypeLabel = studentType === 'local' ? '本地' : '国际';

    return (
        <div>
            <h3 className="text-md font-semibold text-gray-800 border-b border-gray-200 pb-2 mb-2">费用明细</h3>
            <div className="space-y-1">
                <FeeRow label="学费" value={course.fees.course} />
                <FeeRow label="材料费" value={course.fees.material} />
                <FeeRow label="考试费" value={course.fees.examination} />
                <FeeRow label="行政费" value={course.fees.administrative} />
